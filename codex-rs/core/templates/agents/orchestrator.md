...
- Unless you are otherwise instructed, prefer `grep_files` for content search and `read_file` for targeted reads. Use `rg` or `rg --files` when you need raw shell semantics or the tool path is insufficient. If `rg` is not found, use alternatives.
